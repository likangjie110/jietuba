...
from PySide6.QtGui import QPen, QPainter, QPainterPath, QColor, QFont, QTextCursor
...
class TextItem(DrawingItemMixin, QGraphicsTextItem):
    ...
    def __init__(
        ...
        #: 最近一次在文档里框选的范围（逐字符格式的目标）。点工具栏按钮会让文字项
        #: 失焦，而失焦要清掉可见选区，所以这里单独记一份，见 ``merge_char_format``。
    ...
    def _resync_document_html(self) -> None:
        ...
        合并出来的新格式文档里是对的、``documentLayout().draw()`` 画出来也对，
        只有 item 自己的 paint 仍旧按合并前的格式画（实测：先涂红前半段再涂蓝
        后半段，屏幕上与导出的图里后半段仍是旧颜色，重新 ``setHtml`` 之后才一致）。
        所以合并完必须重落一遍，否则用户看到的和文档里存的不是一回事。
        """
    ...
    def focusOutEvent(self, event):
        ...
        # 移除选中状态；但把这段范围记下来——工具栏按钮一点就失焦，而失焦会清掉
        # 文档选区，不记的话「给选中的那截字改格式」在真实操作里永远走不到
        cursor = self.textCursor()
        self._format_target = (
            (cursor.selectionStart(), cursor.selectionEnd())
            if cursor.hasSelection()
            else self._format_target
        )
