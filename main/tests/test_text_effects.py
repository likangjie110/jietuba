...
def test_glyph_outline_lines_up_with_the_painted_text(qapp):
    ...
    # macOS 上跑的是真实字体，边缘按亚像素位置量化，四条边之间的位移会差到 2px；
    # 猜错文档边距（≥3px）或第二行错位这类问题照样过不去
    assert max(shifts) - min(shifts) <= 2, f"字形轮廓和画出来的字不是整体平移关系：{shifts}"
    assert max(abs(shift) for shift in shifts) <= 2
